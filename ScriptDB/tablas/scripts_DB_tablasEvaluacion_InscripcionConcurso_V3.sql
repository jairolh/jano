...
(
  ...
  id_evaluacion_final_reclamo integer,
...
-- Table: concurso.evaluacion_promedio
-- DROP TABLE concurso.evaluacion_promedio;

CREATE TABLE concurso.evaluacion_promedio
(
  id serial NOT NULL,
  id_inscrito integer NOT NULL,
  id_calendario integer NOT NULL,
  puntaje_promedio character varying(8) NOT NULL DEFAULT ''::character varying,
  evaluaciones character varying(512),   	
  fecha_registro character varying(20) ,
  estado character varying(2) NOT NULL DEFAULT 'A'::character varying,   	
  id_reclamacion integer,
  CONSTRAINT pk_id_evaluacion_promedio PRIMARY KEY (id),
  CONSTRAINT fk_promedio_relations_inscrito FOREIGN KEY (id_inscrito)
      REFERENCES concurso.concurso_inscrito (consecutivo_inscrito) MATCH SIMPLE
      ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT fk_promedio_relations_fase FOREIGN KEY (id_calendario)
      REFERENCES concurso.concurso_calendario (consecutivo_calendario) MATCH SIMPLE
      ON UPDATE RESTRICT ON DELETE RESTRICT
)
WITH (
...
ALTER TABLE concurso.evaluacion_promedio
  OWNER TO jano_admin;
