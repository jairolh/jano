...
class Formulario {
    ...
    function formulario() {
		...
		$atributos ['tipoEtiqueta'] = 'inicio';
		$atributos ["leyenda"] = "REGISTRO DE RETIROS";
		echo $this->miFormulario->marcoAgrupacion ( 'inicio', $atributos );
		// --------------------------------------------------------------------------------------------------
		
		// ---------------- CONTROL: Tabla Cargos sin Sara -----------------------------------------------
		
		$cadenaEmpleados = $this->miSql->getCadenaSql ( "buscarRegistroxVinculacion", $_REQUEST['tipoVinculacion']);
		$matrizItems = $primerRecursoDB->ejecutarAcceso ( $cadenaEmpleados, "busqueda" );
		
		
		$longitud = count ( $matrizItems );
		$i = 0;
		
		echo '<table id="tablaReporte" class="display" cellspacing="0" width="100%"> ' . '<thead style="display: table-row-group"><tr><th>' . "CHECK" . '</th><th>' . "DOCUMENTO" . '</th> <th>' . "NOMBRES" . '</th> <th>' . "APELLIDOS" . '</th> <th>' . "TIPO VINCULACIÓN" . '</th><th>' . "FECHA INGRESO" . '</th><th>' . "VER DETALLE" . '</th><th>' . "INACTIVAR" . '</th></tr></thead>
...
//                           </a></center> </td>";
		...
		// -----------------CONTROL: Botón ----------------------------------------------------------------
		$esteCampo = 'botonCancelarRetiros';
		$atributos ["id"] = $esteCampo;
		...
		// -----------------CONTROL: Botón ----------------------------------------------------------------
		$esteCampo = 'botonRegistrarRetiros';
		$atributos ["id"] = $esteCampo;
		$atributos ["tabIndex"] = $tab;
		$atributos ["tipo"] = 'boton';
		// submit: no se coloca si se desea un tipo button genérico
		...
		$valorCodificado .= "&opcion=registrarRetiros";
		/**
