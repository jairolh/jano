...
class Formulario {
    ...
    function formulario() {
        ...
        $esteCampo = 'naturalezaTipoVinculacion';
        $atributos ['id'] = $esteCampo;
        $atributos ['nombre'] = $esteCampo;
        $atributos ['tipo'] = 'text';
        $atributos ['estilo'] = 'jqueryui';
        ...
        $cadenaSqlDetalle = $this->miSql->getCadenaSql("consultarLeyesTipoVinculacion", $matrizItems[$_REQUEST['variable']][0]);

        $matrizDatosLeyes = $primerRecursoDB->ejecutarAcceso($cadenaSqlDetalle, "busqueda", $matrizItems[$_REQUEST['variable']][0], "consultarLeyesParametros");
        ...
        $matrizItemsLey = $primerRecursoDB->ejecutarAcceso($atributos ['cadena_sql'], "busqueda");

        $atributos['matrizItems'] = $matrizItemsLey;
...
// ---------------- CONTROL: Cuadro de Texto --------------------------------------------------------
        ...
        echo $this->miFormulario->campoCuadroTexto($atributos);
        // --------------- FIN CONTROL : Cuadro de Texto --------------------------------------------------
        $cadenaSqlDetalle = $this->miSql->getCadenaSql("consultarRubros", $matrizItems[$_REQUEST['variable']][0]);

        $matrizDatosRubros = $primerRecursoDB->ejecutarAcceso($cadenaSqlDetalle, "busqueda", $matrizItems[$_REQUEST['variable']][0], "consultarLeyesParametros");

        $i = 0;
        $cadenaSelectMultiple1 = '';

        while ($i < count($matrizDatosRubros)) {
            $cadenaSelectMultiple1 = $cadenaSelectMultiple1 . $matrizDatosRubros[$i]['id'] . ',';
            $i++;
        }

        $cadenaSelectMultiple1 = substr($cadenaSelectMultiple1, 0, -1);
        ...
        $matrizRubros = $primerRecursoDB->ejecutarAcceso($atributos ['cadena_sql'], "busqueda");

        $atributos['matrizItems'] = $matrizRubros;
        ...
        unset($atributos);
        // ---------------- CONTROL: Cuadro de Texto --------------------------------------------------------
        $esteCampo = 'cargaSelectMultiple1';
        $atributos ['id'] = $esteCampo;
        $atributos ['nombre'] = $esteCampo;
        $atributos ['tipo'] = 'hidden';
        $atributos ['estilo'] = 'jqueryui';
        ...
        $atributos ['valor'] = $cadenaSelectMultiple1;
        $atributos ['deshabilitado'] = false;
        $atributos ['maximoTamanno'] = '';
        $tab ++;
