...
class BotonHtml extends HtmlBase{
    ...
    private function boton($datosConfiguracion) {
        ...
        if ($this->atributos [self::TIPO] == "boton") {
            ...
            if (!isset ($this->atributos [self::MODAL])){
      				$this->cadenaBoton .= "type='submit' ";
      			}else {
      				$this->cadenaBoton .= "type='button' ";
      			}
            if (isset ( $this->atributos ['modal'] ) && $this->atributos ['modal']!='') {
      				$this->cadenaBoton .= 'data-toggle="modal"'.'data-target="#'.$this->atributos [self::MODAL].'"';
      			}
            if (isset ( $this->atributos [self::ESTILOBOTON] ) && $this->atributos [self::ESTILOBOTON] != "") {
      				$this->cadenaBoton .= "class='" . $this->atributos [self::ESTILOBOTON] . "' ";
      			}
            $this->cadenaBoton .= ">\n";
        }
