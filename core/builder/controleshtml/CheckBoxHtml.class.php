...
class CheckBoxHtml extends HtmlBase{
    ...
    function campoCuadroSeleccion($atributos) {
        ...
        $this->cadenaHTML .= $this->checkBox($this->atributos);
        if (! isset ( $atributos [self::SINDIVISION] )) {
            $this->cadenaHTML .= "\n</div>\n";
        }
    ...
    function checkBox($misAtributos) {
        ...
        $this->miOpcion = "";
        $this->miOpcion .= self::HTMLLABEL . "'" . $this->atributos [self::ID] . " ' class='justificado'>";
        ...
        $this->miOpcion .= self::HTMLCLASS . "'";

        // --------------Atributo class --------------------------------
        $this->miOpcion .= $this->atributos [self::ESTILO] . " ";

        // Si se utiliza jQuery-Validation-Engine
        if (isset($this->atributos ["validar"])) {
            $this->miOpcion .= "validate[" . $this->atributos ["validar"] . "]'" ;
        }
