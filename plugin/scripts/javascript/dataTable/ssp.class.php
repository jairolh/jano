...
class SSP {
	 ...
	 */
	static function simple($request, $conn, $table, $primaryKey, $columns, $myJoin, $myWhere, $enlace = '') {
...
// 		exit ();
		...
		if ($enlace != '') {
			...
			foreach ( $data as $dat ) {
				...
				$variable = "pagina=" . $enlace ['pagina']; // pendiente la pagina para modificar parametro
				$variable .= "&opcion=" . $enlace ['opcion'];
				// $variable .= "&usuario=" . $miSesion->getSesionUsuarioId ();
				
				$parametros = $enlace ['parametros'];
				
				foreach ( $parametros as $key => $values ) {
					
					$variable.="&".$key."=".$dat[$values];
				}
				
				echo $variable;exit;
				
				
// 				$variable .= "&numero_orden=" . $dat [$parametros ['id']];
// 				$variable .= "&tipo_orden=" . $dat [$]
